/**
 * Mirrors src/utils/uiaRecordingSessionToMacro.ts + uiaRecordingHints.ts for Electron main.
 * Converts UiaPeek SignalR recording payloads into flaui.* macro steps.
 */

type RecordingUiHints = {
  automation_id?: string;
  name?: string;
  control_type?: string;
};

function extractRecordingUiHints(payload: unknown): RecordingUiHints {
...
function buildSelectorFromHints(hints: RecordingUiHints): string | null {
  const parts: string[] = [];
